...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        #initize `self.embed_to_hidden_weight` and `self.embed_to_hidden_bias
        W1 = nn.Parameter(torch.empty(self.n_features * self.embed_size, self.hidden_size))
        b1 = nn.Parameter(torch.empty(self.hidden_size))
        self.embed_to_hidden_weight = nn.init.xavier_uniform_(W1)
        self.embed_to_hidden_bias = nn.init.uniform_(b1)

        # dropout
        self.dropout = nn.Dropout(p = self.dropout_prob)

        # Declare `self.hidden_to_logits_weight` and `self.hidden_to_logits_bias`.
        W2 = nn.Parameter(torch.empty(self.hidden_size, self.n_classes))
        b2 = nn.Parameter(torch.empty(self.n_classes)) 
        self.hidden_to_logits_weight = nn.init.xavier_uniform_(W2)
